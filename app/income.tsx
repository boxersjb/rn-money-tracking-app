...
export default function IncomeScreen() {
  ...
  // ฟังก์ชันดึงข้อมูลเพื่อคำนวณยอดสรุปด้านบน
  ...
  // ฟังก์ชันบันทึกข้อมูลเงินเข้า
  const handleSave = async () => {
    // Validate UI เบื้องต้น
    if (!detail.trim()) {
      Alert.alert('แจ้งเตือน', 'กรุณากรอกรายการเงินเข้า');
      return;
    }
    ...
    try {
      // สร้างวันที่ปัจจุบันรูปแบบ YYYY-MM-DD
        ...
        .insert([
          {
            ...
            category: 'income', // สามารถปรับเป็น dynamic ได้ในอนาคต
            note: detail,
      ...
      // เมื่อบันทึกสำเร็จ
      Alert.alert('สำเร็จ', 'บันทึกรายรับเรียบร้อยแล้ว');
      
      // เคลียร์ข้อมูลบนหน้าจอ
      setDetail('');
      setAmount('');
      
      // อัปเดตยอดเงินสรุปด้านบนใหม่
      fetchSummary();
  ...
  // ฟังก์ชันแปลงวันที่ปัจจุบันเป็นภาษาไทย
    ...
    >
      <ScrollView showsVerticalScrollIndicator={false} style={{marginTop: -80, zIndex: 2}} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={styles.formContainer}>
          ...
          <Text style={styles.formTitle}>เงินเข้า</Text>

          {/* Input: รายการเงินเข้า */}
          <View style={styles.inputWrapper}>
            <Text style={styles.floatingLabel}>รายการเงินเข้า</Text>
            <TextInput
          ...
          {/* Input: จำนวนเงินเข้า */}
          <View style={styles.inputWrapper}>
            <Text style={styles.floatingLabel}>จำนวนเงินเข้า</Text>
            <TextInput
              style={styles.textInput}
              placeholder="0.00"
              placeholderTextColor="#B0B0B0"
          ...
          >
            {isSubmitting ? (
              ...
              <Text style={styles.submitButtonText}>บันทึกเงินเข้า</Text>
            )}
          </TouchableOpacity>
        </View>
...
const styles = StyleSheet.create({
  ...
  // --- Header & Summary Styles (Shared with Home) ---
  headerBackground: {
  ...
  floatingLabel: {
    ...
    top: -10,
    left: 15,
    backgroundColor: '#F8F9FA', // ต้องตรงกับสีพื้นหลังของ container หลัก
    paddingHorizontal: 5,
    fontFamily: 'Kanit_400Regular',
    fontSize: 12,
    color: '#666',
